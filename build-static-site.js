...
const notebooksWithErrors = [];
for (const notebook of notebooks) {
    try {
        execSync(`.venv/bin/jupyter-nbconvert --to html --output-dir=dist/${notebook.location} --execute ${notebook.path}`);
    } catch(error) {
        console.error(`Failed to convert notebook ${notebook.name}:`, error.message);
        notebooksWithErrors.push(notebook.link);
    }
}
...
notebooks.filter(n => !notebooksWithErrors.includes(n.link)).forEach(notebook => addLinkToNavigation(notebook));
